    ...
    override fun getGlucoseStatusData(allowOldData: Boolean): GlucoseStatus? {
        ...
        if (sizeRecords == 1) {
            ...
            return GlucoseStatus(
                ...
                dura_ISF_average = now.recalculated,
        ...
        var oldavg: Double = now.recalculated
        ...
        for (i in 0 until sizeRecords) {
            ...
            sumBG += then.recalculated
            sumBG2 += then.recalculated * then.recalculated
            sumxy += then.recalculated * minutesL
        }
        ...
        if (sizeRecords > 3) {
            ...
            for (i in 0 until sizeRecords) {
                ...
                tiLast = ti
                val bg = then.recalculated / scaleBg
                sx += ti
                sx2 += Math.pow(ti, 2.0)
                sx3 += Math.pow(ti, 3.0)
                sx4 += Math.pow(ti, 4.0)
                ...
                if (D != 0.0) {
                    ...
                    for (j in 0..i) {
                        val before = data[j]
                        sSquares += Math.pow(before.recalculated / scaleBg - yMean, 2.0)
                        val deltaT: Double = (before.timestamp - time0) / 1000.0 / scaleTime
                        val bgj: Double = a * Math.pow(deltaT, 2.0) + b * deltaT + c
                        sResidualSquares += Math.pow(before.recalculated / scaleBg - bgj, 2.0)
                    }
                    var rSqu = 0.64
                    if (sSquares != 0.0) {
